...
average_bound_inner_bt = zeros(1, length(alpha_levels));
average_bound_outer_bt = zeros(1, length(alpha_levels));

median_ratio_inner_bt = zeros(1, length(alpha_levels));
median_ratio_outer_bt = zeros(1, length(alpha_levels));

median_ratio_outer_root_bt = zeros(1, length(alpha_levels));
median_ratio_inner_root_bt = zeros(1, length(alpha_levels));

coverage_curves_inner_bt = zeros(1, length(alpha_levels));
coverage_curves_outer_bt = zeros(1, length(alpha_levels));
...
for J = 1:1
    ...
    total_outer_bt = zeros(1, length(alpha_levels));
    total_inner_bt = zeros(1, length(alpha_levels));

    total_bound_outer_bt = zeros(1, length(alpha_levels));
    total_bound_inner_bt = zeros(1, length(alpha_levels));

    total_ratio_outer_bt = zeros(1, length(alpha_levels));
    total_ratio_inner_bt = zeros(1, length(alpha_levels));

    for K = 1:length(alpha_levels)
        % for K = 10
        K
        threshold_inner_bt = prctile(max_vals_inner_all_bt, 100 * (1 - alpha_levels(K)));
        threshold_outer_bt = prctile(max_vals_inner_all_bt, 100 * (1 - alpha_levels(K)));
        ...
        median_ratio_outer_root_bt(:,K) = median(sqrt(save_ratios_outer_bt), 2) + median_ratio_outer_bt(:,K);
        median_ratio_inner_root_bt(:,K) = median(sqrt(save_ratios_inner_bt), 2) + median_ratio_inner_bt(:,K);
    end

    coverage_curves_outer_bt = total_outer_bt/500 + coverage_curves_outer_bt;
    coverage_curves_inner_bt = total_inner/500 + coverage_curves_inner_bt;
